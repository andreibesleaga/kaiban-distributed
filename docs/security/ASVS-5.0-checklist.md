...
|-------------|------------------------|---------|--------|
| **Validation & sanitization** | Validate all untrusted input | `a2a-input-validation.ts`: `agentId` ≤64 `/^[\w-]+$/`, `instruction` ≤10 000 chars; rejects `*`; W3C `traceparent` regex-validated before use (`TraceContext.sanitizeTraceHeaders`, shared by both drivers) | ✅ |
...
| **Self-protection** | Rate limiting / quotas | Per-IP rate limits; per-agent token budget; **global cross-agent rate/cost limiter** (`src/economics/*` — `RateCostLimiter`, pre-step `CostReservation`, `detectSpendAnomaly`; `ECONOMICS_*`, default-OFF) | ✅ (opt-in; turnkey fleet auto-wiring → v2.1) |
...
- The global cross-agent rate/cost limiter (for massive fan-out) is **built in v2.0** (`src/economics/*`, Phase E / ADR-019) but ships **default-OFF and not auto-wired into a fleet budget**; turnkey Redis-backed fleet wiring is deferred to v2.1 (see `docs/roadmap/V2.1-ROADMAP.md` §2).
