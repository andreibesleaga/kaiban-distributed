/**
 * Global Research Run Logger
 *
 * Wraps the shared console logger and accumulates a structured JSON record
 * of every step in the global research pipeline (system info, per-task
 * tokens/cost/answer, errors and the final outcome).
 * Flushed to `runs/` at the end of each run.
 */
import { mkdir, writeFile } from 'fs/promises';
import path from 'path';
import { createLogger } from '../../src/shared';

export const log = createLogger('GlobalResearch');
...
export class RunLogger {
  private readonly record: ResearchRunLog;

  constructor(query: string, gatewayUrl: string, driverType: string, numSearchers: number, contextId: string) {
    this.record = {
  ...
  finish(outcome: ResearchRunLog['outcome']): void {
  ...
  async flush(dir: string): Promise<string> {
    const target = path.resolve(
      process.cwd(),
      dir,
      `${new Date().toISOString().replace(/[:.]/g, '-')}-${slugify(this.record.query || 'research-run')}.json`,
    );
