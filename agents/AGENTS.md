...
```yaml
project_name: "kaiban-distributed"
description: "Distributed Agentic AI via Messaging Layer and Tasks State based KaibanJS Framework and individual AI nodes with external interfaces"
repo_url: "https://github.com/andreibesleaga/kaiban-distributed"
...
deployment_target: "Docker/K8s | Vercel | AWS Lambda | VPS"
...
test_single: "npm test --filter <file>"

# Build for production
build: "npm run build"

# Type checking (must pass before any PR)
typecheck: "npm tsc --noEmit"
...
Functional vs OOP:
  Prefer functional pure functions

Error handling:
  Use Result<T, E> type pattern
...
Use: dotenv | .env.vault | AWS Secrets Manager | HashiCorp Vault
```
...
Agents must use authoritative sources. Never guess or hallucinate based on probabilities and no authoritative content.
...
```
  - "This project must use docker files and local usage of all systems components (egf: Kafka, Confluence, etc, - must exist and be able to be run for the whole system integration and e2e testing)"
  - "System is based on microservices architecture and KaibanJS framework wrapping it"
  - "All final tests must run e2e for all parts of the system and capabilities"
  - "Questions should be asked on all architecture and implementation decisions"
  - "This is a High-level abstraction system that can be used independently and connected to other systems, KaibanJS is the framework that wraps it, it is not a UI framework, and must integrate with the kaibanjs UI board for visualization and management of deployed agentic AI agent nodes and other systems as DIFY js, via components, A2A, MCP, kafka messaging pub/sub, etc"
```
