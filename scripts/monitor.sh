...
KAFKA_CONTAINER="${KAFKA_CONTAINER:-blog-team-kafka-1}"
...
print_header() {
  ...
  echo -e "${DIM}  Driver: ${MESSAGING_DRIVER}   Press Ctrl-C to stop${NC}"
  echo ""
}
...
# ── Stream: Kafka Lags ────────────────────────────────────────────────────────
stream_kafka_lags() {
  echo -e "${YELLOW}[kafka]${NC} Polling Kafka consumer lags every ${QUEUE_POLL_SEC}s..."
  while true; do
    sleep "${QUEUE_POLL_SEC}"
    if ! docker exec "${KAFKA_CONTAINER}" echo "ok" &>/dev/null; then continue; fi

    local groups=("kaiban-group" "kaiban-group-researcher" "kaiban-group-writer" "kaiban-group-editor")
    local summary=""
    for g in "${groups[@]}"; do
      local lag
      # Extract LAG (column 6) from consumer group description
      lag=$(docker exec "${KAFKA_CONTAINER}" kafka-consumer-groups --bootstrap-server "${KAFKA_BROKERS}" --describe --group "$g" 2>/dev/null | awk '/[0-9]/ {sum+=$6} END {print sum}')
      [[ -z "$lag" ]] && lag="?"
      summary+="  ${g}: lag=${lag}"$'\n'
    done
    echo -e "${YELLOW}$(ts) [kafka lags]${NC}\n${DIM}${summary}${NC}"
  done
}
...
if [[ "${MESSAGING_DRIVER}" == "bullmq" ]]; then
  (stream_bullmq_events  2>&1 | sed "s/^/[bull]   /") &
  PIDS+=($!)

  (stream_queue_depths   2>&1 | sed "s/^/[queue]  /") &
  PIDS+=($!)
elif [[ "${MESSAGING_DRIVER}" == "kafka" ]]; then
  (stream_kafka_lags     2>&1 | sed "s/^/[kafka]  /") &
  PIDS+=($!)
fi
