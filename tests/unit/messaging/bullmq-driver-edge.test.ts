...
vi.mock('bullmq', () => ({
  ...
  Worker: vi.fn().mockImplementation(function (name: string, processor: (job: { data: unknown }) => Promise<void>) {
...
describe('BullMQDriver — TLS constructor (line 19 branch)', () => {
  ...
  it('merges TLS config into connection when tls is provided', () => {
    ...
    const driver = new BullMQDriver({ connection: { host: 'redis', port: 6380 }, tls });
    // Trigger Queue creation to expose the stored config
    void driver.publish('q', { taskId: 't', agentId: 'a', data: {}, timestamp: 0 });
    ...
    expect((opts.connection['tls'] as Record<string, unknown>)['ca']).toBe(tls.ca);
    expect((opts.connection['tls'] as Record<string, unknown>)['rejectUnauthorized']).toBe(true);
  });

  it('does NOT include tls in connection when tls is omitted', async () => {
    const driver = new BullMQDriver({ connection: { host: 'redis', port: 6379 } });
    await driver.publish('q', { taskId: 't', agentId: 'a', data: {}, timestamp: 0 });
    const opts = vi.mocked(Queue).mock.calls[0][1] as { connection: Record<string, unknown> };
    expect(opts.connection['tls']).toBeUndefined();
  });

  it('TLS driver publishes successfully using the merged connection config', async () => {
    const tls = { ca: Buffer.from('ca'), cert: Buffer.from('c'), key: Buffer.from('k'), rejectUnauthorized: false };
    const driver = new BullMQDriver({ connection: { host: 'redis', port: 6380 }, tls });
    await driver.publish('queue', { taskId: 'task-tls', agentId: 'a', data: {}, timestamp: 0 });
    expect(mockQueueAdd).toHaveBeenCalledWith('task-tls', expect.objectContaining({ taskId: 'task-tls' }));
  });
});
...
describe('BullMQDriver — multi-queue isolation', () => {
  ...
  it('creates separate Queue instances for different queue names', async () => {
    ...
    await driver.publish('q1', { taskId: 't1', agentId: 'a', data: {}, timestamp: 0 });
    await driver.publish('q2', { taskId: 't2', agentId: 'a', data: {}, timestamp: 0 });
  ...
  it('uses taskId as the BullMQ job name', async () => {
    const driver = new BullMQDriver(cfg);
    await driver.publish('q', { taskId: 'my-task-123', agentId: 'a', data: {}, timestamp: 0 });
  ...
  it('worker processor passes full payload including all fields', async () => {
    ...
    await driver.subscribe('q', async (p) => { received.push(p); });
    const processor = vi.mocked(Worker).mock.calls[0][1] as (job: { data: unknown }) => Promise<void>;
    const payload = { taskId: 't', agentId: 'a', data: { foo: 'bar' }, timestamp: 999, traceHeaders: {} };
    await processor({ data: payload });
    expect(received[0]).toMatchObject({ taskId: 't', data: { foo: 'bar' }, timestamp: 999 });
  });
});
...
describe('BullMQDriver — publish payload enrichment', () => {
  ...
  it('adds traceHeaders to the published payload', async () => {
    const driver = new BullMQDriver(cfg);
    await driver.publish('q', { taskId: 't', agentId: 'a', data: { x: 1 }, timestamp: 0 });
  ...
  it('preserves original payload fields alongside traceHeaders', async () => {
    const driver = new BullMQDriver(cfg);
    const original = { taskId: 'orig-task', agentId: 'agent-x', data: { key: 'val' }, timestamp: 12345 };
