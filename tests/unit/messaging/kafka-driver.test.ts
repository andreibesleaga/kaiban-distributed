...
const mockProducer = { connect: mockProducerConnect, disconnect: mockProducerDisconnect, send: mockSend };
const mockConsumer = { connect: mockConsumerConnect, disconnect: mockConsumerDisconnect, subscribe: mockConsumerSubscribe, run: mockRun };

vi.mock('kafkajs', () => ({
  Kafka: vi.fn().mockImplementation(function () {
    return { producer: vi.fn().mockReturnValue(mockProducer), consumer: vi.fn().mockReturnValue(mockConsumer) };
  }),
}));
...
describe('KafkaDriver', () => {
  ...
  beforeEach(() => { vi.clearAllMocks(); driver = new KafkaDriver({ brokers: ['l:9092'], clientId: 'c', groupId: 'g' }); });

  it('publish() connects and sends with trace headers', async () => {
    await driver.publish('t', { taskId: 'x', agentId: 'a', data: {}, timestamp: 0 });
    expect(mockProducerConnect).toHaveBeenCalledOnce();
    const sentValue = JSON.parse(mockSend.mock.calls[0][0].messages[0].value) as { traceHeaders: unknown };
    expect(sentValue.traceHeaders).toBeDefined();
  });

  it('producer not reconnected on second publish', async () => {
    await driver.publish('t', { taskId: 'x', agentId: 'a', data: {}, timestamp: 0 });
    await driver.publish('t', { taskId: 'y', agentId: 'a', data: {}, timestamp: 0 });
    expect(mockProducerConnect).toHaveBeenCalledOnce();
  });
  ...
  it('subscribe() connects consumer', async () => {
    ...
    expect(mockConsumerSubscribe).toHaveBeenCalledWith({ topic: 't', fromBeginning: false });
  ...
  it('eachMessage extracts trace context and invokes handler', async () => {
    ...
    const runCb = mockRun.mock.calls[0][0] as { eachMessage: (m: { message: { value: Buffer | null } }) => Promise<void> };
    const p = { taskId: 'x', agentId: 'a', data: {}, timestamp: 0, traceHeaders: {} };
    await runCb.eachMessage({ message: { value: Buffer.from(JSON.stringify(p)) } });
  ...
  it('eachMessage skips when message.value is null', async () => {
    const handler = vi.fn();
    await driver.subscribe('t', handler);
    const runCb = mockRun.mock.calls[0][0] as { eachMessage: (m: { message: { value: null } }) => Promise<void> };
  ...
  it('eachMessage handles payload without traceHeaders (covers ?? {} branch)', async () => {
    const handler = vi.fn().mockResolvedValue(undefined);
    await driver.subscribe('t', handler);
    const runCb = mockRun.mock.calls[0][0] as { eachMessage: (m: { message: { value: Buffer } }) => Promise<void> };
