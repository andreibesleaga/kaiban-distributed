...
let capturedProcessor: ((job: { data: unknown }) => Promise<void>) | null =
  null;
...
describe("BullMQDriver — traceparent header validation", () => {
  ...
  async function subscribeAndGetProcessor(
    handler = vi.fn(),
  ): Promise<(job: { data: unknown }) => Promise<void>> {
  ...
  it("passes a valid traceparent through to extractTraceContext", async () => {
    ...
    const calledHeaders = mockExtractTraceContext.mock.calls[0]?.[0] as Record<
      string,
      string
    >;
