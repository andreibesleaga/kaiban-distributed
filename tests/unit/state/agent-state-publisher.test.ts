...
describe("AgentStatePublisher", () => {
  ...
  it("publishIdle() starts heartbeat that re-publishes state periodically", () => {
    ...
    const lastMsg = JSON.parse(mockPublish.mock.calls.at(-1)![1] as string) as {
  ...
  it("wrapHandler() updates currentStatus so heartbeat reflects EXECUTING during task", async () => {
    ...
    const executingHeartbeat = calls.find(
      (c) => c.agents?.[0]?.status === "EXECUTING" && !c.tasks,
    );
  ...
  it("wrapHandler() publishes EXECUTING on start, DONE on completion", async () => {
    ...
    const calls = mockPublish.mock.calls.map((c) => JSON.parse(c[1] as string));
    expect(
      calls.find((c) => c.agents?.[0]?.status === "EXECUTING")?.agents[0]
        .currentTaskId,
    ).toBe("t1");
    expect(
      calls.find((c) => c.tasks?.[0]?.status === "DONE")?.tasks[0].result,
    ).toBe("result text");
  });
  ...
  it("wrapHandler() shows answer text when result is a KaibanHandlerResult (duck-typing)", async () => {
    ...
    const wrapped = publisher.wrapHandler(
      vi.fn().mockResolvedValue({ answer: "hello", score: 9 }),
    );
    await wrapped({
      taskId: "t4",
      agentId: "researcher",
    ...
    const calls = mockPublish.mock.calls.map((c) => JSON.parse(c[1] as string));
    const doneTask = calls.find((c) => c.tasks?.[0]?.status === "DONE")
      ?.tasks[0];
  ...
  it("wrapHandler() publishes ERROR/BLOCKED on throw", async () => {
    ...
    await expect(
      wrapped({ taskId: "t2", agentId: "researcher", timestamp: 0, data: {} }),
    ).rejects.toThrow("LLM failed");
    const calls = mockPublish.mock.calls.map((c) => JSON.parse(c[1] as string));
    expect(
      calls.find((c) => c.agents?.[0]?.status === "ERROR")?.tasks[0].status,
    ).toBe("BLOCKED");
  });
