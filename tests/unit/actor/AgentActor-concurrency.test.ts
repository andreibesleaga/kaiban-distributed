...
describe("AgentActor — concurrency", () => {
  ...
  it("two actors on same queue both receive tasks via competing consumers pattern", async () => {
    ...
    const actor1 = new AgentActor("agent-1", d1, "shared-q", async (p) => {
      results1.push(p.taskId);
      return "";
    });
    const actor2 = new AgentActor("agent-1", d2, "shared-q", async (p) => {
      results2.push(p.taskId);
      return "";
    });
  ...
  it("failed task is retried then routed to DLQ; next task still processes normally", async () => {
    ...
    const publishCalls = (driver.publish as ReturnType<typeof vi.fn>).mock
      .calls;
    const failedCall = publishCalls.find(
      (c) => c[0] === "kaiban-events-failed",
    );
    const completedCall = publishCalls.find(
      (c) => c[0] === "kaiban-events-completed",
    );
