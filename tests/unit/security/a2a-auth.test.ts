...
async function getA2AAuth(): Promise<typeof import('../../../src/infrastructure/security/a2a-auth')> {
...
describe('a2a-auth', () => {
  ...
  it('throws on missing Authorization header (undefined)', async () => {
    ...
    expect(() => verifyA2AToken(undefined)).toThrow('Missing Authorization header');
  });

  it('throws on Authorization header without Bearer prefix', async () => {
    const { verifyA2AToken } = await getA2AAuth();
    expect(() => verifyA2AToken('Token abc123')).toThrow('Missing Authorization header');
  });
  ...
  it('throws when A2A_JWT_SECRET is not set during verify', async () => {
    ...
    delete process.env['A2A_JWT_SECRET'];
    expect(() => verifyA2AToken(`Bearer ${token}`)).toThrow('A2A_JWT_SECRET not set');
  });
});
