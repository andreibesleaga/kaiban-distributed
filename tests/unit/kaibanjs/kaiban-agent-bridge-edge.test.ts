...
describe("kaiban-agent-bridge — Task construction from MessagePayload", () => {
  ...
  it("passes instruction as Task description", async () => {
    ...
    await h({
      ...basePayload,
      data: { instruction: "Research quantum computing" },
    });
    const taskArg = vi.mocked(Task).mock.calls[0]?.[0] as {
      description: string;
    };
  ...
  it("merges context into description when present", async () => {
    ...
    await h({
      ...basePayload,
      data: { instruction: "Edit", context: "DRAFT: hello" },
    });
    const taskArg = vi.mocked(Task).mock.calls[0]?.[0] as {
      description: string;
    };
  ...
  it("description is just instruction when context is empty", async () => {
    ...
    await h({
      ...basePayload,
      data: { instruction: "Do something", context: "" },
    });
    const taskArg = vi.mocked(Task).mock.calls[0]?.[0] as {
      description: string;
    };
...
describe("kaiban-agent-bridge — env API keys without tokenProvider", () => {
  ...
  it("includes multiple API keys from env when set", async () => {
    ...
    await h(basePayload);
    const env = (
      vi.mocked(Team).mock.calls[0]?.[0] as { env: Record<string, string> }
    ).env;
