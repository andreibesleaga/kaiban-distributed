...
describe('TraceContext', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(propagation.extract).mockReturnValue({} as ReturnType<typeof propagation.extract>);
  });
  ...
  it('extractTraceContext() calls propagation.extract with the carrier', () => {
    ...
    expect(propagation.extract).toHaveBeenCalledWith(expect.anything(), carrier);
  });

  it('extractTraceContext() returns the context from propagation.extract', () => {
    const mockCtx = { spanContext: () => 'test' } as unknown as ReturnType<typeof propagation.extract>;
