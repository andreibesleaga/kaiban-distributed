...
function buildSDKSpies(shutdownPromise: Promise<void> = Promise.resolve()): {
...
describe("initTelemetry — exporter selection", () => {
  ...
  it("uses OTLPTraceExporter when exporterEndpoint is provided", () => {
    initTelemetry({
      serviceName: "svc",
      exporterEndpoint: "http://otel:4318/v1/traces",
    });
  ...
  it("constructs NodeSDK with auto-instrumentations result", () => {
    ...
    const sdkArgs = vi.mocked(NodeSDK).mock.calls[0][0] as Record<
      string,
      unknown
    >;
