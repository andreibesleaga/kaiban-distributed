...
async function createTask(agentId: string, driver?: IMessagingDriver): Promise<{
  ok: boolean;
  ...
  return { ok: true, result: val.result as { taskId: string; status: string; agentId: string } };
}
...
describe('Blog-Team orchestrator agentIds pass A2A validation', () => {
  ...
  it.each(blogAgentIds)('agentId "%s" is accepted and publishes to correct queue', async (agentId) => {
...
describe('Global-Research orchestrator agentIds pass A2A validation', () => {
  ...
  it.each(globalAgentIds)('agentId "%s" is accepted and publishes to correct queue', async (agentId) => {
    const driver = makeMockDriver();
    const res = await createTask(agentId, driver);
    expect(res.error).toBeUndefined();
    expect(res.result?.status).toBe('QUEUED');
