...
describe('GatewayApp', () => {
  ...
  beforeEach(() => { gateway = new GatewayApp(new A2AConnector(testCard)); });
  ...
  it('POST /a2a/rpc with text/plain returns 415', async () => {
    const res = await request(gateway.app)
      .post('/a2a/rpc').set('Content-Type', 'text/plain').send('data');
  ...
  it('POST /a2a/rpc connector error returns 500', async () => {
    ...
    vi.spyOn(errorConnector, 'handleRpc').mockResolvedValueOnce({
      ok: false,
      error: { code: 'ERR', message: 'fail', name: 'Error' } as unknown as DomainError,
    });
