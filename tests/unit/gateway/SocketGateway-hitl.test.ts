...
vi.mock('socket.io', () => ({
  Server: vi.fn().mockImplementation(function () {
    return { adapter: vi.fn(), emit: mockEmit, close: mockIoClose, on: mockIoOn, use: vi.fn() };
  }),
}));
...
describe('SocketGateway — HITL edge cases', () => {
  ...
  function getHitlHandler(socket: MockSocket): (payload: unknown, ack?: (r: { ok: boolean; error?: string }) => void) => void {
    const calls = socket.on.mock.calls as Array<[string, (...args: unknown[]) => void]>;
    const hitlCall = calls.find((c) => c[0] === 'hitl:decision');
    return hitlCall![1] as (payload: unknown, ack?: (r: { ok: boolean; error?: string }) => void) => void;
  }

  function connectSocket(): { socket: MockSocket; hitl: ReturnType<typeof getHitlHandler> } {
  ...
  it('rejects payload with missing taskId', () => {
    ...
    expect(ack).toHaveBeenCalledWith({ ok: false, error: 'invalid taskId or decision' });
  ...
  it('rejects invalid decision value not in VALID_DECISIONS', () => {
    ...
    expect(ack).toHaveBeenCalledWith({ ok: false, error: 'invalid decision value: INVALID' });
    expect(mockPublish).not.toHaveBeenCalled();
  });
  ...
  it('respects custom validHitlDecisions list', async () => {
    ...
    expect(ack1).toHaveBeenCalledWith({ ok: false, error: 'invalid decision value: PUBLISH' });
