...
describe('Live E2E: Global Research Swarm (Real LLM + Docker)', { timeout: 600_000 }, () => {
  ...
  it('Scenario 1 — Golden Path: search → write → governance → editorial → PUBLISHED', () => {
    const { stdout, stderr, status } = runOrchestrator(baseEnv({
    ...
    expect(stdout).toMatch(/AUTO_PUBLISH.*auto-approving|RESEARCH PUBLISHED/i);
  ...
  it('Scenario 2 — Governance: structured compliance review with score and recommendation', () => {
    const { stdout, status } = runOrchestrator(baseEnv({
    ...
    const rejectedByGovernance = /Governance review REJECTED|Workflow stopped/i.test(stdout);
  ...
  it('Scenario 3 — ResearchContext: metadata (active nodes, tokens, cost) reported', () => {
    ...
    expect(stdout).toMatch(/ResearchContext populated with \d+ search results/);
