...
export async function setup(): Promise<void> {
  ...
  console.log(
    "[Live E2E] ── Starting Docker Compose (global-research) ─────────",
  );
  execSync(
  ...
  console.log(
    "[Live E2E] ── Waiting for gateway /health ──────────────────────",
  );
  ...
  console.log(
    "[Live E2E] ── Waiting 12s for agents to register ───────────────",
  );
...
export async function teardown(): Promise<void> {
  ...
  try {
    execSync(`docker compose -f ${COMPOSE_FILE} --env-file ${ENV_FILE} down`, {
      cwd: ROOT,
      stdio: "inherit",
    });
  } catch {
