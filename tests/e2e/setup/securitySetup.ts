...
function assertDockerAvailable(): void {
  try {
    ...
    throw new Error(
      "[E2E Security] Docker not found — install Docker Desktop and try again",
    );
  }
}
...
function isRedisReady(): boolean {
  const result = spawnSync(
    ...
    [
      "exec",
      SECURITY_REDIS_CONTAINER,
      "redis-cli",
      "-a",
      SECURITY_TEST_REDIS_PASSWORD,
      "--no-auth-warning",
      "ping",
    ],
...
function removeContainerIfExists(): void {
  ...
  spawnSync("docker", ["stop", SECURITY_REDIS_CONTAINER], {
    stdio: "pipe",
    timeout: 10_000,
  });
  spawnSync("docker", ["rm", "-f", SECURITY_REDIS_CONTAINER], {
    stdio: "pipe",
    timeout: 10_000,
  });
}
...
export async function setup(): Promise<void> {
  ...
  console.log(
    "[E2E Security] Removing any leftover security-test Redis container...",
  );
  ...
  console.log(
    `[E2E Security] Starting Redis on port ${SECURITY_REDIS_PORT} with password...`,
  );
  execSync(
    [
      "docker run -d",
  ...
  console.log(
    "[E2E Security] Waiting for Redis (with password) to be ready...",
  );
  await waitForRedis();
...
export async function teardown(): Promise<void> {
  ...
  try {
    execSync(`docker stop ${SECURITY_REDIS_CONTAINER}`, {
      stdio: "pipe",
      timeout: 10_000,
    });
    execSync(`docker rm -f ${SECURITY_REDIS_CONTAINER}`, {
      stdio: "pipe",
      timeout: 10_000,
    });
  } catch {
