...
import { issueBoardToken, verifyBoardToken } from '../../src/infrastructure/security/board-auth';
...
async function postRpc(baseUrl: string, headers: Record<string, string> = {}): Promise<Response> {
...
describe('E2E: A2A JWT authentication', () => {
  ...
  it('passes through when A2A_JWT_SECRET is not set', async () => {
    ...
    expect(res.status).toBe(200);
    await new Promise<void>((resolve, reject) => srv2.close((e) => (e ? reject(e) : resolve())));
  });
});
...
describe('E2E: Channel signing round-trip', () => {
  ...
  it('unwrapVerified returns null when payload is tampered', () => {
    ...
    const wrapped = wrapSigned(payload);
    // Tamper the envelope
    const envelope = JSON.parse(wrapped) as { payload: Record<string, unknown>; sig: string; ts: number };
...
describe('E2E: GatewayApp hardening', () => {
  ...
  it('agent.status returns correct JSON-RPC 2.0 structure', async () => {
    ...
    expect(res.status).toBe(200);
    const body = await res.json() as { jsonrpc: string; id: number; result: Record<string, unknown> };
    expect(body.jsonrpc).toBe('2.0');
