...
describe("E2E: Distributed Event Isolation", () => {
  ...
  it("task completions keyed by taskId: order does not matter", async () => {
    ...
    // Publish two tasks
    await pubDriver.publish(queue, {
      taskId: t1,
      agentId: `iso-agent-${prefix}`,
      data: {},
      timestamp: Date.now(),
    });
    await pubDriver.publish(queue, {
  ...
  it("channel-signed messages under concurrent load: all unwrap correctly", async () => {
    ...
    const indices = new Set(
      received.map((m) => (m as Record<string, unknown>)["index"]),
    );
  ...
  it("queue isolation: task on queue-A not received by actor on queue-B", async () => {
    ...
    const actorA = new AgentActor(
      `agentA-${prefix}`,
      conDriverA,
      queueA,
      async (p) => {
        receivedA.push(p.taskId);
        return "";
      },
    );
    const actorB = new AgentActor(
      `agentB-${prefix}`,
      conDriverB,
      queueB,
      async (p) => {
        receivedB.push(p.taskId);
        return "";
      },
    );
    ...
    await pubDriver.publish(queueA, {
      taskId: `${prefix}-only-A`,
      agentId: `agentA-${prefix}`,
      data: {},
      timestamp: Date.now(),
    });
