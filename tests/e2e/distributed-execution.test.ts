...
describe('E2E: Distributed Task Execution', () => {
  ...
  it('Scenario 1: Task published to queue is consumed by AgentActor', async () => {
    ...
    const actor = new AgentActor('e2e-agent-1', consumerDriver, 'e2e-queue-agent1');
    await actor.start();
    ...
    await publisherDriver.subscribe('kaiban-events-completed', async (payload) => {
  ...
  it('Scenario 2: Fault tolerance - retry on failure publishes to DLQ', async () => {
    ...
    const actor = new AgentActor('e2e-agent-fail', driver, 'e2e-queue-fail', failingHandler);
    await actor.start();
