...
const RESEARCH_RESULT = JSON.stringify({
  answer: 'AI agents are reshaping software development in 2025. Key facts: 60% of Fortune 500 use LLM APIs, agent frameworks grew 400% YoY.',
  inputTokens: 200,
  outputTokens: 150,
  estimatedCost: 0.012,
});
...
describe('E2E: Blog Team Full Flow (BullMQ + HITL)', () => {
  ...
  it('Scenario 1 — Golden Path: 3-stage pipeline completes with HITL PUBLISH', async () => {
    ...
    // Spin up agents
    await spawnAgent(drivers, 'researcher', RESEARCHER_Q, async () => RESEARCH_RESULT);
    ...
    await oracleDriver.publish(RESEARCHER_Q, {
      taskId: researchTaskId, agentId: 'researcher', timestamp: Date.now(),
    ...
    const researchResult = getHandlerResult(researchPhase.success.get(researchTaskId));
    const researchParsed = JSON.parse(String(researchResult)) as { answer: string };
    ...
    await oracleDriver.publish(WRITER_Q, {
      ...
      data: { instruction: 'Write a blog post', context: researchParsed.answer },
    });
    ...
    await publishState({
      ...
      metadata: { totalTokens: 1300, estimatedCost: 0.062, endTime: Date.now() },
    });
  ...
  it('Scenario 2 — HITL REVISE: editor recommends REVISE, revision task created, then PUBLISH', async () => {
    ...
    const revisionResult = getHandlerResult(revisionPhase.success.get(revisionTaskId));
    const revisionParsed = JSON.parse(String(revisionResult)) as { answer: string };
    ...
    await publishState({
      ...
      metadata: { totalTokens: 1900, estimatedCost: 0.088, endTime: Date.now() },
    });
  ...
  it('Scenario 3 — HITL REJECT: human rejects at editorial stage → STOPPED state published', async () => {
    ...
    await spawnAgent(drivers, 'editor', EDITOR_Q, async () => EDIT_REJECT_RESULT);
  ...
  it('Scenario 4 — Agent failure: researcher exhausts retries → DLQ, BLOCKED state published', async () => {
    ...
    await publishState({
      agents: [{ agentId: 'researcher', name: 'Ava', role: 'News Researcher',
        status: 'ERROR', currentTaskId: researchTaskId }],
      tasks: [{ taskId: researchTaskId, title: 'Research: AI agents', status: 'BLOCKED',
        assignedToAgentId: 'researcher', result: 'ERROR: LLM API unavailable' }],
    });
    ...
    const taskDelta = stateWatcher.deltas.find(d =>
      Array.isArray(d['tasks']) &&
      (d['tasks'] as Array<Record<string, unknown>>).some(t =>
        t['taskId'] === researchTaskId && t['status'] === 'BLOCKED',
      ),
    );
  ...
  it('Scenario 5 — State metadata: tokens, cost, and timestamps published correctly', async () => {
    ...
    await publishState({ metadata: { totalTokens: 350, estimatedCost: 0.012 } });
    await publishState({ metadata: { totalTokens: 1300, estimatedCost: 0.04 } });
    ...
    const runningDelta = stateWatcher.deltas.find(d => d['teamWorkflowStatus'] === 'RUNNING');
    expect(runningDelta).toBeDefined();
    const runningMeta = runningDelta?.['metadata'] as Record<string, unknown> | undefined;
    expect(runningMeta?.['startTime']).toBeGreaterThan(0);
    expect(runningMeta?.['totalTokens']).toBe(0);

    const finishedDelta = stateWatcher.deltas.find(d => d['teamWorkflowStatus'] === 'FINISHED');
    expect(finishedDelta).toBeDefined();
    const finishedMeta = finishedDelta?.['metadata'] as Record<string, unknown> | undefined;
  ...
  it('Scenario 6 — REVISE then REJECT: revision submitted, second HITL rejects it', async () => {
    ...
    await publishState({
      teamWorkflowStatus: 'STOPPED',
      tasks: [{ taskId: revisionTaskId, status: 'BLOCKED', assignedToAgentId: 'writer',
