...
describe("E2E: KafkaDriver", () => {
  ...
  it("publishes a message and the consumer receives it", async () => {
    ...
    const producer = new KafkaDriver({
      brokers: KAFKA_BROKERS,
      clientId: "e2e-producer",
      groupId: "e2e-group-pub",
    });
    const consumer = new KafkaDriver({
      brokers: KAFKA_BROKERS,
      clientId: "e2e-consumer",
      groupId: `e2e-group-sub-${Date.now()}`,
    });
    drivers.push(producer, consumer);

    const received: MessagePayload[] = [];
    await consumer.subscribe(topic, async (payload) => {
  ...
  it("unsubscribe stops consumer from receiving further messages", async () => {
    ...
    const driver = new KafkaDriver({
      brokers: KAFKA_BROKERS,
      clientId: "e2e-unsub",
      groupId: `e2e-unsub-${Date.now()}`,
    });
    drivers.push(driver);

    const received: MessagePayload[] = [];
    await driver.subscribe(topic, async (p) => {
      received.push(p);
    });
    await new Promise((r) => setTimeout(r, 500));

    await driver.unsubscribe(topic);
    await driver.publish(topic, {
      taskId: "after-unsub",
      agentId: "a",
      data: {},
      timestamp: Date.now(),
