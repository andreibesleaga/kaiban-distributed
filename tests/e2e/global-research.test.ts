...
import type {
  ResearchContext,
  SearchResult,
} from "../../examples/global-research/types";
...
describe("E2E: Global Research Swarm Pipeline (BullMQ)", () => {
  ...
  it("Scenario 1 — Golden Path: 4 searchers → writer → reviewer approves → editor approves", async () => {
    ...
    await Promise.all(
      searchTaskIds.map((taskId) =>
        oracleDriver.publish(SEARCHER_QUEUE, {
          taskId,
          agentId: "searcher",
          timestamp: Date.now(),
    ...
    const reviewResult = getHandlerResult(
      reviewPhase.success.get(reviewTaskId),
    );
  ...
  it("Scenario 2 — Chaos Simulation: ~20% crash rate on searchers, all tasks complete via retry", async () => {
    ...
    await Promise.all(
      taskIds.map((taskId) =>
        oracleDriver.publish(QUEUE, {
          taskId,
          agentId: "searcher",
          timestamp: Date.now(),
          data: { subTopic: taskId },
        }),
      ),
    );
  ...
  it("Scenario 3 — Partial failure: 1/4 searchers fails, writer uses remaining 3/4", async () => {
    ...
    await spawnAgents(drivers, NUM, "searcher", QUEUE, async (p) => {
      if (p.taskId === FAILING_TASK)
        throw new Error("permanent search failure");
      return `Search result for ${p.taskId}`;
    });
  ...
  it("Scenario 4 — Reviewer rejects: governance failure stops workflow", async () => {
    ...
    await oracleDriver.publish(REVIEWER_QUEUE, {
      ...
      timestamp: Date.now(),
      data: {
        instruction: "Review this report",
        draft: "Sample research report with issues",
      },
    });
  ...
  it("Scenario 6 — Late-joining searcher: processes tasks published before connecting", async () => {
    ...
    await spawnAgents(
      ...
      "searcher",
      QUEUE,
      async (p) => `Late result for ${p.taskId}`,
    );
  ...
  it("Scenario 7 — ResearchContext serializes through Redis intact", async () => {
    ...
    await spawnAgents(drivers, 1, "writer", QUEUE, async (p) => {
      ...
      return JSON.stringify({
        synthesised: `Synthesised report for ${payloadCtx.originalQuery}`,
        updatedContext,
      });
    });
    ...
    expect(receivedCtx!.rawSearchData[0].sourceUrl).toBe(
    ...
    const parsed = JSON.parse(String(handlerResult ?? "{}")) as {
      synthesised: string;
